...
function MapController({ center }: { center: [number, number] }) {
  ...
  useEffect(() => {
    if (currentMap && center) { currentMap.setView(center, 15); }
  }, [center, currentMap]);
  return null;
}
...
export default function AdminMap() {
  ...
  useEffect(() => {
    ...
    fetchLocation();
    const interval = setInterval(fetchLocation, 5000); // Check every 5s
    ...
    <div style={{ height: '100vh', width: '100%' }} className="bg-slate-950 font-sans">
      <div className="p-4 bg-blue-900 text-white flex justify-between items-center border-b border-blue-800 shadow-2xl">
        ...
        <div className="text-right flex items-center gap-4">
          <div className={`w-2 h-2 rounded-full transition-colors ${isPulsing ? 'bg-emerald-400 shadow-[0_0_10px_#34d399]' : 'bg-slate-600'}`} />
          <div>
